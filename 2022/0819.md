...
- 一つのファイルに存在する同名のlocalシンボルはおそらく重複するのではと思われる
- 昨日は実態を伴うグローバルなシンボルのため重複するんじゃないかということ

### わかってないこと

- （ちょっと脱線だが） 本実験のシンボルに `0000000100008000 S _ppp` とある。これは実態をともなっていないのでリンクエラーにならなかったと思われるが、S type になってリンクに失敗しない過程や意味がよくわかってない
